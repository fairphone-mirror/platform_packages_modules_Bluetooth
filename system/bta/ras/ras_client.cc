  ...
  void OnRemoteData(const tBTA_GATTC_NOTIFY& evt, std::shared_ptr<RasTracker> tracker) {
    ...
    if (!is_last) {
      ...
      if (p_dev_rec && (p_dev_rec->conn_params.peripheral_latency >= 2)) {
        log::info("Low power mode timer started: {}", p_dev_rec->conn_params.peripheral_latency);
        SetTimeOutAlarm(tracker, kFollowingSegmentTimeoutMs_lowpower, FOLLOWING_SEGMENT);
      } else {
